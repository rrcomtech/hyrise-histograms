...
# Extra Doc by Rob and Tob

Extra implemented Environment variables, that can be used to adjust settings:

- `HISTOGRAM`: Defines which histogram is used. For a list of available histograms look at `src/lib/statistics/table_statistics.cpp` (line 50ish)
- `CARDINALITIES`: Defines the filename, where the histogram statistics will be put. If none given, no file will be outputted.
- `BUILD_TIME`: Defines the filename, where the histogram build performance will be put. If none given, no file will be outputted.

Each of the env variables have to be defined via:
```
