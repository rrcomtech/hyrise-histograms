...
BenchmarkRunner::BenchmarkRunner(const BenchmarkConfig& config,
  ...
  if (BUILD_TIMES_FILE) {
    if (!std::filesystem::exists(BUILD_TIMES_FILE)) {
      ...
      obuild_times_file << "HISTOGRAM_NAME,COLUMN_DATA_TYPE,COLUMN_ID,TOTAL_COUNT,BIN_COUNT,BUILD_TIME,THREAD_COUNT,SAMPLING_RATE\n";
...
void BenchmarkRunner::run() {
  ...
  if (CARDINALITIES_FILENAME) {
    auto file_exist = std::filesystem::exists(CARDINALITIES_FILENAME);
    std::ofstream out(CARDINALITIES_FILENAME, std::ios_base::app);

    if (!file_exist) {
      out << "benchmark,histogram,query,operator_count,operator_type,real_input,real_output,estimated_input,estimated_output,sampling_rate\n";
    }
