...
template <typename T>
class EquiWidthHistogram : public AbstractHistogram<T> {
 public:
  using AbstractHistogram<T>::AbstractHistogram;

  EquiWidthHistogram(std::vector<T>&& bin_minima, std::vector<T>&& bin_maxima,
  ...
  static std::shared_ptr<EquiWidthHistogram<T>> from_column(const Table& table, const ColumnID column_id,
  ...
  bool operator==(const EquiWidthHistogram<T>& rhs) const;
...
// For gtest
template <typename T>
std::ostream& operator<<(std::ostream& stream, const EquiWidthHistogram<T>& histogram) {
  stream << histogram.description() << std::endl;
  return stream;
}

// EXPLICITLY_DECLARE_DATA_TYPES(EquiWidthHistogram);
